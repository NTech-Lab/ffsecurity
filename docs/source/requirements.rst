...
.. important::
   Для улучшения качества работы системы при использовании камер(ы) с разрешением более ``1920x1080p`` настоятельно рекомендуется установить версию FindFace Security с обработкой видео на GPU. Для получения GPU-версии компонента ``video-worker`` обратитесь к менеджеру Ntech Lab или в службу технической поддержки (info@ntechlab.com).
