...
#. Если есть запущенные процессы ``video-worker``, нужно либо пересоздать камеры в веб-интерфейсе, либо изменить значение параметра ``router_url`` в job-заданиях, заменив приставку ``http://`` на ``https://``. Это можно сделать с помощью команды, аналогичной следующей:
